...
export const postUser = async (for_user) => {
    await axios.post('http://localhost:3009/user/create', { email:for_user.email, username: for_user.username, password: for_user.password })
...
export const loginUser = async (for_user) => {
    return await axios.post('http://localhost:3009/user/login', { email: for_user.email, password: for_user.password })
